...
const Clickmainpage = () => {
    ...
    const fetchEmail = async () => {
      try {
        ...
        if (Array.isArray(res.data.emails) && res.data.emails.length > 0) {
          const extractedEmails = res.data.emails.map(
            (emailObj) => emailObj._id
          );
          setEmails(extractedEmails);
        } else {
          console.warn(
            "Emails data is empty or not an array:",
            res.data.emails
          );
  ...
  const uploadImage = async (index, imageNumber, imageurl) => {
    ...
    try {
      ...
      setActiveTablayout(true); // Switch back to components tab
    } catch (err) {
        ...
        <div className="app-container">
          ...
          {showSendModal && (
            <SendbulkModal
              ...
              bgColor={bgColor}
              temname={templateName} // Pass templateName to SendbulkModal
          ...
          {showSendexcelModal && (
            <SendexcelModal
              ...
              bgColor={bgColor}
              temname={templateName} // Pass templateName to Sendexcelmail
