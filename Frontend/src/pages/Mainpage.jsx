...
const handleTemplateSelect = (template) => {
 ...
 const addMultiImage = () => {
  ...
  const deleteContent = async (index) => {
    ...
    const s3Key = decodeURIComponent(fileToDelete.url.split('.amazonaws.com/')[1]);
    if (!s3Key) return;
  
    try {
      const res = await fetch(`${apiConfig.baseURL}/api/stud/file?key=${encodeURIComponent(s3Key)}`, {
        method: 'DELETE',
      });
  ...
  const handleSaveButton = () => {
    ...
    if (templateName && user && user.id && previewContent) {
      axios
        .post(`${apiConfig.baseURL}/api/stud/template`, { temname:templateName, userId: user.id,previewContent,bgColor,camname:campaign.camname })
        .then((res) => {
...
const sendscheduleEmail = async () => {
  ...
  const sendEmail = async () => {
    ...
    try {
      const recipients = emailData.recipient.split(",").map((email) => email.trim());
      ...
      console.log("Emails sent. Final Progress:", progress);
    } catch (error) {
